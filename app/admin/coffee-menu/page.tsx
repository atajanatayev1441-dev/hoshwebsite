    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
      ...
      >
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-sage-600 dark:text-sage-300 mb-1">Цена (TMT)</label>
...
export default function AdminCoffeeMenuPage() {
  ...
  const fetchAll = useCallback(async () => {
    const [cats, menuItems] = await Promise.all([
      // Fetch coffee categories only
      fetch('/api/admin/categories?venue=coffee').then((r) => r.json()),
      // Fetch all items (filtered client-side by categoryId)
      fetch('/api/coffee/menu').then((r) => r.json()),
    ])
    setCategories(cats)
    setItems(menuItems)
  ...
  const addCategory = async () => {
    ...
    // POST to coffee categories — venue=coffee is set by the API
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        <div className="relative w-8 h-8 rounded-full overflow-hidden bg-[#3e4e3a] flex items-center justify-center" style={{ padding: '2px' }}>
          <Image src="/images/hoscoffee.png" alt="HOŞ Coffee" width={32} height={32} className="object-contain" />
        </div>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs font-semibold text-sage-600 dark:text-sage-300 mt-0.5">
                        {new Intl.NumberFormat('ru-RU').format(item.price)} TMT
                      </p>
                    </div>
