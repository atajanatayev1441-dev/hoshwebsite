...
export default function AdminCoffeeOrdersPage() {
  ...
  const updateStatus = async (id: number, status: string) => {
    setUpdating(id)
    try {
      await fetch(`/api/coffee/orders/${id}/status`, {
        method: 'PUT',
                ...
                >
                  ...
                  <div className="flex items-center gap-3">
                    <span className="font-semibold text-sage-700 dark:text-sage-200 text-sm">
                      {new Intl.NumberFormat('ru-RU').format(order.totalAmount)} TMT
                    </span>
                    <ChevronDown
